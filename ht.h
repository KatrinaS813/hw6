...
private:
    ...
    HASH_INDEX_T load_ = 0; 
    HASH_INDEX_T size_ = 0; 
    const double resizeAlpha_; 
...
HashTable<K,V,Prober,Hash,KEqual>::HashTable(
    double resizeAlpha, const Prober& prober, const Hasher& hash, const KEqual& kequal)
       :  hash_(hash), kequal_(kequal), prober_(prober), resizeAlpha_(resizeAlpha), mIndex_(0), table_(CAPACITIES[0], nullptr)
{
...
{
    if((double) load_ / CAPACITIES[mIndex_] >= resizeAlpha_){
        this->resize(); 
    }
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    table_.clear(); 
    table_ = std::vector<HashItem*>(newCapacity, nullptr); 
    load_ = 0; 
    size_ = 0;
    ...
    for(HashItem* item : oldTable){
        ...
        if((item->deleted) != true){
            HASH_INDEX_T loc = this->probe((item->item).first);
